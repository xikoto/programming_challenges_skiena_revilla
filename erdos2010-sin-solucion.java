...
class Main
{
	...
	public static void main( String args[] )
	{
		...
		while( ++escenario <= escenarios ) {
			...
			Queue<Autor> cola = new LinkedList<Autor>();
			...
			while( !cola.isEmpty()){
				current = cola.remove();
				numErdos = current.getErdosNumber()+1;
				
				for(Autor next : current.getCoautores()){
					if(numErdos < next.getErdosNumber()){
						next.setErdosNumber(numErdos);
						if(!next.getEnCola()){
							next.setEnCola(true);
							cola.add(next);
						}
